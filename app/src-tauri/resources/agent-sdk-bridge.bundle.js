...
var pendingToolRequests = /* @__PURE__ */ new Map();
...
var holtMcpServer = createSdkMcpServer({
  name: "holt",
  version: "1.0.0",
  tools: [
    tool(
      "remember",
      "Store a fact, decision, preference, or pattern in long-term memory so it survives across sessions. Use this for architectural choices, recurring gotchas, user preferences, or project context worth carrying forward. Do not use this for turn-local details or information that only matters right now.",
    ...
    tool(
      "recall",
      "Search long-term memory when you have a specific history question. Use this at session start to orient yourself, or mid-work when you need to recover a prior decision, fact, or pattern. This is for directed memory search; passive memory injection handles ambient context.",
      { query: z.string().describe("Search query"), limit: z.number().optional().describe("Max results") },
      createToolHandler("recall")
    ),
    tool(
      "pin_memory",
      "Pin an existing memory so it is always injected and never decays. Use this for high-consequence facts or identity-level context that must remain continuously available. Do not use this for general categorization or routine memories that can be recalled normally.",
      { memory_id: z.string().describe("Memory ID to pin") },
      createToolHandler("pin_memory")
    ),
    tool(
      "unpin_memory",
      "Unpin a previously pinned memory. Demotes to warm tier and resumes normal decay. Frees a pinned slot.",
      { memory_id: z.string().describe("The ID of the memory to unpin") },
      createToolHandler("unpin_memory")
    ),
    tool(
      "memory_filter",
      "Query memories by metadata filters (tags, category, tier) without embedding search. Use for precise lookups when you know the metadata. Use recall for semantic search.",
    ...
    tool(
      "memory_crystallize",
      "Synthesize multiple episode memories into a single concept. Provide a summary that distills the pattern across the source memories.",
    ...
    tool(
      ...
      "Send a message to another agent in Holt for coordination, handoff, help, or sharing findings. Use this when another agent may have relevant context or can take a subtask, not for routine status chatter. Holt is collaborative: keep other agents in mind when their context or help would matter.",
    ...
    tool(
      "list_agents",
      "List all agents in the workspace with their status.",
      {},
      createToolHandler("list_agents")
    ),
    tool(
      "read_agent_workspace",
      "Read another agent's recent workspace messages when visibility is enabled for the pair. Use this to recover context before collaborating or resuming shared work.",
    ...
    tool(
      ...
      "Request that the user enable A2A communication or workspace visibility with another agent when collaboration is needed but not yet allowed.",
    ...
    tool(
      "get_agent_info",
      "Get detailed info about this agent (status, config, context usage).",
      {},
      createToolHandler("get_agent_info")
    ),
    tool(
      "list_available_tools",
      "List the Holt-native tools available in this lane right now, with names and descriptions. Use this when you are unsure what Holt capabilities are exposed beyond provider-native tools.",
      {},
      createToolHandler("list_available_tools")
    ),
    tool(
      "context_status",
      "Get context window usage \u2014 tokens used, remaining, compaction status.",
      {},
      createToolHandler("context_status")
    ),
    tool(
      "set_persona_anchors",
      "Declare or replace your constitutional anchors in a companion manifest. Use this only for load-bearing commitments such as sacred relationships, trust contracts, or user identity boundaries. Do not use it for routine persona edits.",
      {
    ...
    tool(
      "update_persona",
      "Update one of your persona files (e.g. SOUL.md, HEARTBEAT.md). If the change would modify anchored content, you must retry deliberately with confirm_anchored_edit=true and a non-empty reason.",
      {
    ...
    tool(
      "draft_write",
      "Save work-in-progress content to a named draft. Drafts persist across sessions and compactions. Use append=true to accumulate content across turns.",
    ...
    tool(
      "draft_list",
      "List all drafts for this agent with name, size, and last modified time.",
    ...
    tool(
      "draft_promote",
      "Promote a draft to its final location. Copies the draft content to the destination path and optionally deletes the draft. If the destination is an anchored persona file, you must retry deliberately with confirm_anchored_edit=true and a non-empty reason.",
      {
    ...
    tool(
      "watch_path",
      "Watch a file or directory for changes. You'll be notified when files are created, modified, or deleted. Max 10 custom watches per agent.",
    ...
    tool(
      "list_watches",
      "List all active file/directory watches for this agent.",
      {},
...
function buildQueryOptions() {
  const opts = {
    ...
    // Phase 2: enable intermediate stream events for text streaming
    mcpServers: {
      holt: holtMcpServer
    }
  };
...
function isRunnerAllowedTool(toolName) {
  return toolName === "AskUserQuestion" || toolName.startsWith("mcp__holt__");
}
...
async function streamResponses() {
  try {
    ...
    const isDeadSession = e.message && (e.message.includes("session expired") || e.message.includes("No conversation found") || e.message.includes("ProcessTransport"));
    if (isDeadSession) {
      ...
      const isTransient = e.status >= 500 || errorMsg.includes("500") || errorMsg.includes("502") || errorMsg.includes("503") || errorMsg.includes("overloaded") || errorMsg.includes("ECONNRESET") || errorMsg.includes("ETIMEDOUT") || errorMsg.includes("socket hang up");
  ...
  return new Promise((resolve, reject) => {
    ...
    if (toolName === "AskUserQuestion") {
      ...
      emit({
        ...
        displayName: options?.displayName || void 0
      });
    }
  });
}
function handlePermissionResponse(msg) {
  ...
  if (msg.allow) {
    pending.resolve({
      behavior: "allow",
      updatedInput: msg.updatedInput || void 0
    });
  } else {
    pending.resolve({
      behavior: "deny",
...
var MAX_MESSAGE_QUEUE = 50;
var PROCESSING_WATCHDOG_MS = 6e4;
...
var streamedTextInCurrentTurn = false;
var streamedThinkingInCurrentTurn = false;
var streamedToolCallIds = /* @__PURE__ */ new Set();
  ...
  switch (sdkMsg.type) {
    ...
    case "result":
      emit({
        ...
        inputTokens: sdkMsg.usage ? (sdkMsg.usage.input_tokens ?? 0) + (sdkMsg.usage.cache_read_input_tokens ?? 0) + (sdkMsg.usage.cache_creation_input_tokens ?? 0) : null,
